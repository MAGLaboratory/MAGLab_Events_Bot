...
def test_render_writes_fresh_image(monkeypatch, tmp_path):
    ...
    def fake_generate(samples, output: Path, **_kwargs) -> Path:
...
def test_render_serves_cached_when_regeneration_fails(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(synoptic, "generate_synoptic_image", lambda *_args, **_kwargs: None)
...
def test_render_can_refuse_cached_image_after_regeneration_failure(monkeypatch, tmp_path):
    target = tmp_path / "synoptic.png"
    target.write_bytes(b"cached-bytes")
    monkeypatch.setattr(synoptic, "generate_synoptic_image", lambda *_args, **_kwargs: None)

    result = synoptic.get_synoptic_image_bytes(
        _samples(),
        target,
...
def test_async_render_returns_bytes(monkeypatch, tmp_path):
    ...
    def fake_generate(_samples, output: Path, **_kwargs) -> Path:
